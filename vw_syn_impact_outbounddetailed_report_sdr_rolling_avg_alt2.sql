create or replace table vfvrs_dh_lake_bi_tableau_s.vw_syn_impact_outbounddetailed_report_sdr_rolling_avg_alt2_prev as 
select * from vfvrs_dh_lake_bi_tableau_s.vw_syn_impact_outbounddetailed_report_sdr_rolling_avg_alt2;

create or replace table vfvrs_dh_lake_bi_tableau_s.vw_syn_impact_outbounddetailed_report_sdr_rolling_avg_alt2 as
with vw_syn_impact_outbounddetailed_report_sdr as (
...
from vfvrs_dh_lake_bi_syniverse_processed_s.syn_project1_outbounddetailedreport 
...
tbl_vw_syn_impact_outbounddetailed_report_sdr as (
select
...
from vw_syn_impact_outbounddetailed_report_sdr syn_ib_dtl_rpt left outer join ( select * from vf-vrs-datahub.vfvrs_dh_lake_bi_syniverse_processed_s.syn_project1_fxrates where currency_code='EUR') syn_fx
on syn_ib_dtl_rpt.call_date between  syn_fx.effective_date and syn_fx.end_date
),

vw_syn_impact_outbounddetailed_report_sdr_impact_per as (
  select  in_dtl_rpt.my_pmn_tadig_code
,in_dtl_rpt.their_pmn_tadig_code
,in_dtl_rpt.call_type
,in_dtl_rpt.total_charged_volume_mb
,in_dtl_rpt.total_charges_sdr
,in_dtl_rpt.total_charges_eur
,iot_30.total_charges_eur total_charges_eur_30
,in_dtl_rpt.call_date
,iot_30.call_date  call_date_30
,in_dtl_rpt.iot_rate_eur
...
from tbl_vw_syn_impact_outbounddetailed_report_sdr  in_dtl_rpt
left outer join tbl_vw_syn_impact_outbounddetailed_report_sdr  iot_30
...
from vw_syn_impact_outbounddetailed_report_sdr_impact_per
order by  my_pmn_tadig_code, their_pmn_tadig_code, call_date ) alt2
where not exists (select 1
from vfvrs_dh_lake_bi_tableau_s.vw_syn_impact_outbounddetailed_report_sdr_rolling_avg_alt2_prev prev
...
from vfvrs_dh_lake_bi_tableau_s.vw_syn_impact_outbounddetailed_report_sdr_rolling_avg_alt2_prev;
